...
parser.add_argument('--code_size', type=int, default=512, metavar='N',
...
encoder = models.resnet18(pretrained=True).to("cuda")
...
def loss_compute_val(encoded,predicted):
    	#print(encoded.shape)
	...
	plt.figure()
	cmap=plt.cm.Blues
	cm = prod[0].cpu().detach().numpy()
	plt.imshow(cm, interpolation='nearest', cmap=cmap)
	plt.title("title")
	plt.colorbar()
	# tick_marks = np.arange(len(49))
	...
	plt.show()
	import random
	plt.savefig("Mat_rest"+str(epoch)+".png")
...
def validate():
	...
	for batch_idx,(data,_) in enumerate(test_loader):
		...
		with torch.no_grad():
			...
			predicted_output=autoregressor(output)
			#print(predicted_output.shape)
			loss=loss_compute_val(output,predicted_output)
			epoch_loss+=loss.item()
			if batch_idx % args.log_interval == 0:
